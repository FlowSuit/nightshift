...
import{supabase,getChannel}from'./supabase.js'
...
const Hdr=({title,sub,onBack,extra})=>(
  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
    ...
    <span style={{width:50,textAlign:"right"}}>{extra}</span>
  </div>
)
...
function QRDisplay({url}){
  ...
  return src?<img src={src} alt="QR code" style={{width:180,height:180,borderRadius:16}}/>:<div style={{width:180,height:180,background:C.surface2,borderRadius:16}}/>
}
  ...
  return(
    <div>
      ...
      <Card style={{marginBottom:16}}>
        ...
        <Input value={name} onChange={setName} onKey={()=>valid&&(tab==='create'?onCreate(name.trim()):valid&&joinCode.length===4&&onJoin(name.trim(),joinCode.toUpperCase()))} placeholder="Vul je naam in" autoFocus/>
      </Card>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
        {['create','join'].map(t=>(
          <button key={t} onClick={()=>setTab(t)} style={{padding:"14px 0",borderRadius:16,fontWeight:800,fontSize:14,cursor:"pointer",background:tab===t?C.lime:C.surface2,color:tab===t?"#0B0B12":C.muted,border:`1px solid ${tab===t?C.lime:C.line}`}}>
            {t==='create'?'KAMER MAKEN':'KAMER JOINEN'}
          </button>
        ))}
      </div>
      {tab==='create'&&(
        <Btn full disabled={!valid} onClick={()=>valid&&onCreate(name.trim())}>MAAK KAMER</Btn>
      )}
      {tab==='join'&&(
        <div style={{display:"grid",gap:10}}>
          <div>
            <p style={{color:C.muted,fontWeight:700,fontSize:13,marginBottom:8}}>Kamernummer</p>
            <Input value={joinCode} onChange={v=>setJoinCode(v.toUpperCase().slice(0,4))} onKey={()=>valid&&joinCode.length===4&&onJoin(name.trim(),joinCode.toUpperCase())} placeholder="bv. KWRT"/>
          </div>
          <Btn full disabled={!valid||joinCode.length!==4} onClick={()=>onJoin(name.trim(),joinCode.toUpperCase())}>JOIN</Btn>
        </div>
      )}
    </div>
  )
    ...
    <div>
      ...
      <Card style={{marginBottom:16}}>
        ...
        <div style={{display:"grid",gap:6}}>
          {players.map(p=>(
            <div key={p.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",borderRadius:12,background:C.surface2,border:`1px solid ${p.id===players[0]?.id?C.lime+"44":C.line}`}}>
              <span style={{color:C.text,fontWeight:600}}>{p.name}</span>
              {p.id===players[0]?.id&&<Pill color={C.lime}>Host</Pill>}
              {p.id===myId&&p.id!==players[0]?.id&&<Pill color={C.cyan}>Jij</Pill>}
            </div>
          ))}
        </div>
      </Card>
...
// All modes receive: {players, addScore, tiers, onBack, isHost, myId, sendAction, pendingActions, clearActions}
// isHost = can advance game state
// sendAction = send vote/answer to channel
// pendingActions = collected actions from others
...
function Actie({players,addScore,tiers,onBack,isHost,state,setState}){
  ...
  const next=pts=>{
    ...
    else setState({...state,turn:turn+1})
  }
  return(
    <div>
      <Hdr title="VOER UIT OF DRINK" sub={`${rn}/${R.actie}`} onBack={isHost?onBack:null}/>
      <div style={{textAlign:"center",marginBottom:14}}>
        <p style={{color:C.muted,fontWeight:700,fontSize:14}}>Aan de beurt</p>
        <p style={{color:C.lime,fontFamily:"Anton,sans-serif",fontSize:32}}>{sp.name}</p>
      </div>
      <Card style={{marginBottom:18}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
          <Pill color={d.c}>{d.l}</Pill>
          <Pill color={C.gold}>+{o.p||2} PT</Pill>
        </div>
        <p style={{color:C.text,fontWeight:600,fontSize:20,lineHeight:1.35}}>{o.t}</p>
      </Card>
      {isHost&&<div style={{display:"grid",gap:10}}>
        <Btn full onClick={()=>next(o.p||2)}>GEDAAN +{o.p||2} pt</Btn>
        <Btn full dark onClick={()=>next(0)}>GEWEIGERD - drink 3 slokken</Btn>
      </div>}
...
function Wie({players,addScore,tiers,onBack,isHost,myId,sendAction,state,setState}){
  ...
  const castVote=(tid)=>{
    if(!myVote){
      const newVotes={...votes,[myId]:tid}
      sendAction({type:"wie_vote",votes:newVotes})
  ...
  const nextR=()=>{
    ...
    if(!reveal.tie){
      const ws=new Set(reveal.winners.map(w=>w.id))
      players.forEach(p=>{if(ws.has(votes[p.id]))addScore(p.id,2)})
    }
    if(rn>=R.wie)setState({...state,done:true})
    else setState({...state,ronde:ronde+1,phase:"intro",votes:{},straf:null})
  }
  ...
  return(
    <div>
      ...
      {phase==="intro"&&<>
        <Card style={{textAlign:"center",marginBottom:18}}><Pill color={C.cyan}>STEMRONDE</Pill><p style={{color:C.text,fontWeight:700,fontSize:22,lineHeight:1.35,marginTop:16}}>{vraag.t}</p></Card>
        {isHost&&<Btn full color={C.cyan} onClick={startVote}>BEGIN STEMMEN</Btn>}
        {!isHost&&<Card border={C.cyan+"44"} style={{textAlign:"center"}}><p style={{color:C.muted,fontSize:14}}>Wacht op host...</p></Card>}
      </>}
      {phase==="vote"&&<>
        <Card border={C.cyan+"55"} style={{textAlign:"center",marginBottom:18}}>
          <p style={{color:C.text,fontWeight:600,fontSize:17}}>{vraag.t}</p>
        </Card>
        {myVote?<Card border={C.lime+"44"} style={{textAlign:"center",marginBottom:14}}><p style={{color:C.lime,fontWeight:800}}>✓ Stem uitgebracht!</p><p style={{color:C.muted,fontSize:13,marginTop:4}}>Wacht op anderen... ({Object.keys(votes).length}/{players.length})</p></Card>:<>
          <p style={{color:C.text,fontWeight:700,textAlign:"center",marginBottom:12}}>Stem op de speler die het best past:</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {players.map(p=>(
              <button key={p.id} onClick={()=>castVote(p.id)} disabled={p.id===myId} style={{padding:18,borderRadius:18,background:p.id===myId?C.bg:C.surface2,color:p.id===myId?C.line:C.text,fontWeight:700,border:`1px solid ${C.line}`,cursor:p.id===myId?"default":"pointer",opacity:p.id===myId?0.35:1}}>{p.name}{p.id===myId?" (jij)":""}</button>
            ))}
          </div>
        </>}
...
function Koning({players,addScore,tiers,onBack,isHost,state,setState}){
  ...
  const king=players[ronde%players.length]
  const o=kp[ronde%kp.length]
  ...
  return(
    <div>
      ...
      {phase==="pick"&&<>
        ...
        {isHost&&<><p style={{color:C.muted,fontSize:13,textAlign:"center",marginBottom:14}}>Kies het slachtoffer:</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{players.filter(p=>p.id!==king.id).map(p=>(<button key={p.id} onClick={()=>setState({...state,phase:"ch",targetId:p.id})} style={{padding:18,borderRadius:18,background:C.surface2,color:C.text,fontWeight:700,border:`1px solid ${C.line}`,cursor:"pointer"}}>{p.name}</button>))}</div></>}
...
function Nhie({players,addScore,tiers,onBack,isHost,myId,sendAction,state,setState}){
  ...
  const pr=np[ronde%np.length]
  const rn=ronde+1
  const myVote=votes[myId]
  const wel=players.filter(p=>votes[p.id]==="wel")
  const niet=players.filter(p=>votes[p.id]==="niet")
  const allVoted=players.every(p=>votes[p.id])
  const allSame=allVoted&&(wel.length===0||niet.length===0)
  const minority=wel.length<=niet.length?wel:niet
  const majority=wel.length>niet.length?wel:niet
  const alone=allVoted&&minority.length===1

  const castVote=v=>{
    if(myVote)return
    const newVotes={...votes,[myId]:v}
    sendAction({type:"nhie_vote",votes:newVotes})
    setState({...state,votes:newVotes})
  }
  const nextR=sid=>{
    if(allVoted&&!allSame){majority.forEach(p=>addScore(p.id,2));if(alone)addScore(minority[0].id,3);if(sid)addScore(sid,3)}
    if(rn>=R.nhie)setState({...state,done:true})
    else setState({...state,ronde:ronde+1,phase:"intro",votes:{}})
  }
  ...
  return(
    <div>
      ...
      {phase==="intro"&&<>
        <Card border={C.purple+"55"} style={{textAlign:"center",marginBottom:18}}><Pill color={C.purple}>IK HEB NOG NOOIT...</Pill><p style={{color:C.text,fontWeight:700,fontSize:22,lineHeight:1.35,marginTop:16}}>{pr.t}</p></Card>
        {isHost&&<Btn full color={C.purple} onClick={()=>setState({...state,phase:"vote",votes:{}})}>BEGIN STEMMEN</Btn>}
        {!isHost&&<Card border={C.cyan+"44"} style={{textAlign:"center"}}><p style={{color:C.muted,fontSize:14}}>Wacht op host...</p></Card>}
      </>}
      {phase==="vote"&&<>
...
function BesteAntwoord({players,addScore,tiers,onBack,isHost,myId,sendAction,state,setState}){
  ...
  const pr=bp[ronde%bp.length]
  const rn=ronde+1
  const myAnswer=answers[myId]
  const allAnswered=players.every(p=>answers[p.id])

  const submitAnswer=()=>{
    if(!input.trim()||myAnswer)return
    const newAnswers={...answers,[myId]:input.trim()}
    sendAction({type:"ba_answer",answers:newAnswers})
  ...
  return(
    <div>
      ...
      {phase==="intro"&&<>
        <Card border={C.gold+"55"} style={{textAlign:"center",marginBottom:18}}><Pill color={C.gold}>VRAAG</Pill><p style={{color:C.text,fontWeight:700,fontSize:22,lineHeight:1.35,marginTop:16}}>{pr.t}</p></Card>
        {isHost&&<Btn full color={C.gold} onClick={()=>setState({...state,phase:"type",answers:{}})}>BEGIN</Btn>}
        {!isHost&&<Card border={C.cyan+"44"} style={{textAlign:"center"}}><p style={{color:C.muted,fontSize:14}}>Wacht op host...</p></Card>}
      </>}
      {phase==="type"&&<>
        ...
        {myAnswer?<Card border={C.lime+"44"} style={{textAlign:"center",marginBottom:14}}><p style={{color:C.lime,fontWeight:800}}>✓ Antwoord ingevoerd!</p><p style={{color:C.muted,fontSize:13,marginTop:4}}>{Object.keys(answers).length}/{players.length} klaar</p></Card>:<>
          <Input value={input} onChange={setInput} onKey={submitAnswer} placeholder="Typ je antwoord..."/>
          <div style={{marginTop:10}}><Btn full color={C.gold} disabled={!input.trim()} onClick={submitAnswer}>VERZEND</Btn></div>
        </>}
      ...
      {phase==="reveal"&&<>
        <Card border={C.gold+"55"} style={{marginBottom:14}}>
          <Pill color={C.gold}>ANTWOORDEN</Pill>
          <p style={{color:C.muted,fontSize:14,marginTop:8,marginBottom:14}}>{pr.t}</p>
          {players.map(p=>(<div key={p.id} style={{padding:"12px 16px",borderRadius:14,background:C.surface2,marginBottom:8,border:`1px solid ${C.line}`}}><p style={{color:C.muted,fontSize:12,fontWeight:700}}>{p.name}</p><p style={{color:C.text,fontWeight:600,fontSize:16,marginTop:4}}>{answers[p.id]||"..."}</p></div>))}
        </Card>
...
function Versus({players,addScore,tiers,onBack,isHost,state,setState}){
  ...
  const sp=players[ronde%players.length]
  const o=vp[ronde%vp.length]
  const d=MD[o.m||2]
  const opp=players.find(p=>p.id===oppId)
  const winner=players.find(p=>p.id===winnerId)
  const rn=ronde+1
  const dealAmt=(o.m||2)+1
...
function Groep({players,addScore,tiers,onBack,isHost,state,setState}){
  ...
  if(!state)return null
  const{ronde=0,done=false}=state
  const o=gp[ronde%gp.length]
  const rn=ronde+1
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    if(roomParam&&roomParam.length===4){
      document.getElementById("root").setAttribute("data-room",roomParam.toUpperCase())
    }
  },[])
  ...
  const setupChannel=(code,myP,hosting)=>{
    ...
    ch.on("broadcast",{event:"player_action"},({payload})=>{
      ...
      setModeState(prev=>{
        ...
        if(payload.type==="ba_answer")return{...prev,answers:{...prev.answers,...payload.answers}}
        return prev
      })
    })
  ...
  const handleJoin=(name,code)=>{
    ...
    setPlayers([player])
    setScreen("lobby")
    setupChannel(code,player,false)
    window.history.replaceState({},"",`?room=${code}`)
  }
  ...
  const handleLeave=()=>{
    ...
    setCurrentMode(null)
    setModeState(null)
    window.history.replaceState({},"","/"+(window.location.search.includes("room")?"":`?`))
    window.history.replaceState({},"","/")
  }

  const addScore=(id,pts)=>{
    setPlayers(prev=>prev.map(p=>p.id===id?{...p,score:p.score+pts}:p))
  }

  const pickMode=(mode)=>{
    setCurrentMode(mode)
    const initState={turn:0,ronde:0,phase:"intro",done:false,votes:{},answers:{},oppId:null,winnerId:null,drinksLeft:0,drinksGiven:{},d1:null,d2:null,fx:null,val:1,targetId:null,straf:null}
    setModeState(initState)
  }
  ...
  // Check URL room param on first render
  useEffect(()=>{
    const params=new URLSearchParams(window.location.search)
    const r=params.get("room")
    if(r&&r.length===4&&screen==="home"){
      const el=document.getElementById("prefill-room")
